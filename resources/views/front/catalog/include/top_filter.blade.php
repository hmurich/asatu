<div class="restaurants-filtr">
    <form action='{{ action('Front\CatalogController@postAddress') }}' method="post">
        <div class="restaurants-filtr__item restaurants-filtr__item-city">
            <span>Ваше местоположение: {{ (isset($ar_city[$location->city_id]) ? $ar_city[$location->city_id].',' : null) }} {{ $location->address }}</span>
    ...
    <form action='{{ action('Front\CatalogController@getList') }}' method="get">
        <div class="restaurants-filtr__item restaurants-filtr__item--bot">
            ...
            <div class="restaurants-filtr__item__mobile">
                ...
                @if (isset($ar_input["k_name"]) && $ar_input["k_name"])
                    <div class="sort-reiting">
                         Сортировка по рейтингу
                        <div class="sort-reiting__sub">
                             <a href='?k_name='.$ar_input["k_name"].'&sort_name=raiting&sort_asc=0'>по рейтингу</a>
                             <a href='?k_name='.$ar_input["k_name"].'&sort_name=count_view&sort_asc=0'>По просмотрам</a>
                             <a href='?k_name='.$ar_input["k_name"].'&sort_name=price&sort_asc=1'>По цене</a>
                         </div>
                    </div>
                    <input type='hidden' name="k_name" value="{{ (isset($ar_input["k_name"]) ? $ar_input["k_name"] : null) }}">
                @endif
